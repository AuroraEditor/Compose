...
struct MaintainerAuthenticator: AsyncJWTAuthenticator {
    ...
    func authenticate(jwt: Payload, for request: Request) async throws -> Void {
        ...
        // We then check to make sure the user is an maintainer in order to run certain endpoints requirng the admin
        // permission.
        if !User(from: jwt).role.rawValue.contains("maintainer") {
            throw AuthenticationError.invalidPermission
        }
